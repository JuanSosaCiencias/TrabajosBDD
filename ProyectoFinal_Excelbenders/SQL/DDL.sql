...
COMMENT ON COLUMN Atleta.Temporada IS 'Año de temporada que juega el atleta.';
...
COMMENT ON CONSTRAINT Atleta_pkey ON Atleta IS 'Llave primaria de la tabla Atleta.'; 
...
-- Evento
ALTER TABLE Evento
    ADD CONSTRAINT CK_Evento_Precio
    CHECK (Precio >= 0),
    ADD CONSTRAINT CK_Evento_DuracionMax
    CHECK (DuracionMax > 0),
...
COMMENT ON CONSTRAINT CK_Localidad_Aforo ON Localidad IS 'Asegura que el aforo sea un número positivo';
COMMENT ON CONSTRAINT CK_Localidad_Tipo ON Localidad IS 'Limita los tipos de localidad a valores predefinidos (Estadio, Piscina, etc.)';
